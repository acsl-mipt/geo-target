...
public class MavlinkIfDevSourceGenerator
{
    ...
    @NotNull
    private final MavlinkIfDevSourceGeneratorConfiguration config;
    @NotNull
    ...
    {
        MavlinkIfDevSourceGeneratorConfiguration config = new MavlinkIfDevSourceGeneratorConfiguration();
        CmdLineParser parser = new CmdLineParser(config);
        try
        {
            parser.parseArgument(Arrays.asList(args));
            new MavlinkIfDevSourceGenerator(config).generate();
        }
    ...
    public MavlinkIfDevSourceGenerator(@NotNull MavlinkIfDevSourceGeneratorConfiguration config)
    {
        this.config = config;
    }
        ...
        try
        {
            ...
            for (Element field : message.getChildren("field"))
            {
                ...
                fields.add(new StructField(type, fieldNameFormatted, Optional.ofNullable(fieldDescription).map(
                        MavlinkIfDevSourceGenerator::escapeUnaryQuotesString),
                        anEnum));
            }
